...
#python bed_merger.py test.bed 50/0.5 for merging segment within 50bp distance or 0.5 of neighboring segment length (whichever is smaller)
# 
...
#v2.0 allow the adjustment for 50/0.5 length(int)/fraction(float) to avoid anchoring effect by two small events, such as 100-101 and 149-151 won't merge due to their size been adjusted
...
#assign the border according to minlen setting
if "/" in minlen:
    minlen1 = int(re.sub('/.*$','',minlen))
    minlen2 = float(re.sub('^.*/','',minlen))
    print(minlen,"use the minimum of fixed distance of:",minlen1,"or fraction of neighboring segments:",minlen2)
    min_len1 = [minlen1/2] * df.shape[0]
    min_len2 = (df['end'] - df['start']) * minlen2
    min_len = [min(i,j) for i,j in zip(min_len1,min_len2)]
else:
    if "." in minlen:
